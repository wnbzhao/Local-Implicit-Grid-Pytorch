...
if __name__ == "__main__":
    input_ply = 'shapenet_chair.ply'
    v, n = read_point_ply(input_ply)

    query_points = farthest_point_sample(v[np.newaxis, :, :], 10000)
    rand_samples = random_point_sampling(v[np.newaxis, :, :], 10000)

    np.savetxt("fps_samples.txt", query_points[0], fmt='%f', delimiter=';')
    np.savetxt("rand_samples.txt", rand_samples[0], fmt='%f', delimiter=';')
